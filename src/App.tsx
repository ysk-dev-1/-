...
const INIT_DISPATCH = [
  { id:1, shipId:1, companyId:1, forklifts:4, startTime:"08:00", endTime:"17:00", extraHours:2, status:"승인완료",     date:"2026-04-03", unitPrice:50000, rejectReason:"", companyNote:"", origForklifts:4, origStartTime:"08:00", origEndTime:"17:00", origExtraHours:2, modNote:"" },
  { id:2, shipId:2, companyId:2, forklifts:3, startTime:"09:00", endTime:"18:00", extraHours:0, status:"요청중",       date:"2026-04-05", unitPrice:50000, rejectReason:"", companyNote:"", origForklifts:3, origStartTime:"09:00", origEndTime:"18:00", origExtraHours:0, modNote:"" },
  { id:3, shipId:3, companyId:3, forklifts:5, startTime:"07:00", endTime:"16:00", extraHours:3, status:"요청중",       date:"2026-04-07", unitPrice:50000, rejectReason:"", companyNote:"", origForklifts:5, origStartTime:"07:00", origEndTime:"16:00", origExtraHours:3, modNote:"" },
  { id:4, shipId:1, companyId:1, forklifts:2, startTime:"08:00", endTime:"17:00", extraHours:1, status:"승인완료",     date:"2026-03-15", unitPrice:50000, rejectReason:"", companyNote:"", origForklifts:2, origStartTime:"08:00", origEndTime:"17:00", origExtraHours:1, modNote:"" },
  { id:5, shipId:2, companyId:3, forklifts:3, startTime:"07:00", endTime:"16:00", extraHours:0, status:"업체수락",     date:"2026-03-20", unitPrice:50000, rejectReason:"", companyNote:"작업 준비 완료", origForklifts:3, origStartTime:"07:00", origEndTime:"16:00", origExtraHours:0, modNote:"" },
  { id:6, shipId:1, companyId:2, forklifts:2, startTime:"10:00", endTime:"19:00", extraHours:1, status:"요청중",       date:"2026-04-03", unitPrice:50000, rejectReason:"", companyNote:"", origForklifts:2, origStartTime:"10:00", origEndTime:"19:00", origExtraHours:1, modNote:"" },
  { id:7, shipId:4, companyId:1, forklifts:3, startTime:"08:00", endTime:"17:00", extraHours:0, status:"요청중",       date:"2026-04-08", unitPrice:50000, rejectReason:"", companyNote:"", origForklifts:3, origStartTime:"08:00", origEndTime:"17:00", origExtraHours:0, modNote:"" },
  { id:8, shipId:5, companyId:2, forklifts:2, startTime:"09:00", endTime:"18:00", extraHours:2, status:"수정확인요청", date:"2026-04-09", unitPrice:50000, rejectReason:"", companyNote:"", origForklifts:2, origStartTime:"09:00", origEndTime:"18:00", origExtraHours:0, modNote:"실제 작업 연장으로 추가 2시간" },
];
...
const STATUS_STYLE: Record<string,{bg:string,color:string}> = {
  ...
  "수정확인요청": { bg:"#FAEEDA", color:"#854F0B" },
  "수정승인":     { bg:"#EAF3DE", color:"#3B6D11" },
  "수정거절":     { bg:"#FCEBEB", color:"#A32D2D" },
};
...
const COMPANY_TABS    = ["내 업무현황","배차 수락/거절","수정확인","업무기록","업체정보","월별정산"];
...
const getDayName = (s: string) => { const i=new Date(s).getDay(); return DAY_NAMES[i===0?6:i-1]; };
const nowStr = () => new Date().toLocaleString("ko-KR",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"}).replace(/\. /g,"-").replace(".",":").trim();
...
export default function App() {
  ...
  const [completedMonth, setCompletedMonth] = useState("2026-04");
  ...
  const addLog = (userId: number, userName: string, action: string, detail: string) =>
    setLogs(prev=>[{id:prev.length+1, time:nowStr(), userId, userName, action, detail}, ...prev]);
  ...
  const register = () => {
    ...
    const nc = { id:companies.length+1, name:reg.companyName, contact:reg.contact, forklifts:0, manager:reg.manager, address:reg.address, bizNo:reg.bizNo, email:reg.email, vehicles:[] as any[] };
    setCompanies(p=>[...p,nc]);
    setUsers(p=>[...p,{ id:p.length, loginId:reg.loginId, pw:reg.pw, name:reg.name||reg.manager, role:"company", companyId:nc.id }]);
    setShowRegister(false);
    setReg({ loginId:"",pw:"",pw2:"",name:"",companyName:"",manager:"",contact:"",address:"",bizNo:"",email:"" });
    setRegErr(""); alert("가입 완료! 로그인 해주세요.");
  };

  const setStatus = (id: number, status: string, extra: any={}) =>
    setDispatches(ds=>ds.map(d=>d.id===id?{...d,status,...extra}:d));

  const addShip = () => {
    if (!newShip.name||!newShip.eta) return;
    setShips(p=>[...p,{id:p.length+1,...newShip}]);
    setNewShip({name:"",berth:"",eta:"",cargo:""}); setShowAddShip(false);
  };

  const addDispatch = (cu: any) => {
    ...
    const nd = { id:dispatches.length+1, ...newDispatch, shipId:Number(newDispatch.shipId), companyId:Number(newDispatch.companyId), forklifts:Number(newDispatch.forklifts), extraHours:Number(newDispatch.extraHours), unitPrice:Number(newDispatch.unitPrice), status:"요청중", date:ship?.eta||"", rejectReason:"", companyNote:"", origForklifts:Number(newDispatch.forklifts), origStartTime:newDispatch.startTime, origEndTime:newDispatch.endTime, origExtraHours:Number(newDispatch.extraHours), modNote:"" };
    setDispatches(p=>[...p,nd]);
    addLog(cu.id, cu.name, "배차 요청", `${comp?.name} → ${ship?.name} ${nd.forklifts}대`);
  ...
  const saveCompany = () => { setCompanies(p=>p.map(c=>c.id===editingCompany.id?editingCompany:c)); setEditingCompany(null); };

  const deleteCompany = (companyId: number, cu: any) => {
    if (!window.confirm("정말 이 업체와 계정을 삭제하시겠습니까?")) return;
    const comp = companies.find(c=>c.id===companyId);
    setCompanies(p=>p.filter(c=>c.id!==companyId));
    setUsers(p=>p.filter(u=>u.companyId!==companyId));
    setDispatches(p=>p.filter(d=>d.companyId!==companyId));
    addLog(cu.id, cu.name, "업체 삭제", `${comp?.name} 삭제`);
  };

  const addCompanyByAdmin = (cu: any) => {
    ...
    const nc = { id:companies.length+1, name:newCompany.name, contact:newCompany.contact, forklifts:Number(newCompany.forklifts), manager:newCompany.manager, address:newCompany.address, bizNo:newCompany.bizNo, email:newCompany.email, vehicles:[] as any[] };
    setCompanies(p=>[...p,nc]);
    setUsers(p=>[...p,{ id:p.length, loginId:newCompany.loginId, pw:newCompany.pw, name:newCompany.manager, role:"company", companyId:nc.id }]);
    addLog(cu.id, cu.name, "업체 추가", `${nc.name} 등록`);
    setNewCompany({name:"",manager:"",contact:"",email:"",bizNo:"",address:"",forklifts:0,loginId:"",pw:""});
    setNewCompanyErr(""); setShowAddCompany(false);
  };

  const addUserBySuperAdmin = (cu: any) => {
    ...
    setUsers(p=>[...p,nu]);
    addLog(cu.id, cu.name, "업체 추가", `사용자 ${nu.name}(${nu.loginId}) 추가`);
    setNewUser({loginId:"",pw:"",name:"",role:"admin"}); setNewUserErr(""); setShowAddUser(false);
  };

  const submitDispatchEdit = (d: any, cu: any) => {
    const ship = ships.find(s=>s.id===d.shipId);
    setDispatches(ds=>ds.map(x=>x.id===d.id?{ ...x, forklifts:Number(editForm.forklifts), startTime:editForm.startTime, endTime:editForm.endTime, extraHours:Number(editForm.extraHours), origForklifts:d.forklifts, origStartTime:d.startTime, origEndTime:d.endTime, origExtraHours:d.extraHours, modNote, status:"수정확인요청" }:x));
    addLog(cu.id, cu.name, "배차 요청", `완료내역 수정요청: ${ship?.name} - ${modNote}`);
    setEditingDispatchId(null); setModNote("");
  };

  const downloadMonthlyExcel = () => {
    const header = ["날짜","선박명","부두","업체명","지게차(대)","정규시간","추가시간(h)","금액(원)","상태"];
    const items = dispatches.filter(d=>d.date.startsWith(selectedMonth));
    const rows = items.map(d=>{ const ship=ships.find(s=>s.id===d.shipId); const comp=companies.find(c=>c.id===d.companyId); return [d.date,ship?.name||"",ship?.berth||"",comp?.name||"",d.forklifts,`${d.startTime}~${d.endTime}`,d.extraHours,calcAmount(d),d.status]; });
    rows.push(["","","","합계","","","",items.reduce((s,d)=>s+calcAmount(d),0),""]);
  ...
  const s: any = {
    ...
    metric3:   { display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:20 },
    ...
    btnPu:     { background:"#534AB7", color:"#fff" },
    btnGray:   { background:"#f0f0f0", color:"#555" },
    btnGreen:  { background:"#3B6D11", color:"#fff" },
    btnBlue:   { background:"#E6F1FB", color:"#185FA5" },
  ...
  if (!currentUser) {
    if (showRegister) return (
      <div style={s.wrap}>
        <div style={{background:"#1a3a5c",color:"#fff",padding:"14px 20px",borderRadius:"10px 10px 0 0"}}><div style={{fontSize:17,fontWeight:700}}>협력업체 회원가입</div></div>
    ...
    return (
      <div style={s.wrap}>
        <div style={{background:"#1a3a5c",color:"#fff",padding:"14px 20px",borderRadius:"10px 10px 0 0"}}><div style={{fontSize:17,fontWeight:700}}>항구 지게차 배차 관리 시스템</div><div style={{fontSize:12,opacity:.8,marginTop:2}}>Port Forklift Dispatch Management</div></div>
        <div style={{...s.body,maxWidth:420,margin:"30px auto"}}>
          <div style={{...s.card,padding:28}}>
            ...
            <div style={{marginTop:16,background:"#f8f9fa",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#888"}}>
              <b>테스트 계정</b><br/>시스템관리자: superadmin / super1234<br/>관리자: admin / 1234<br/>한진물류: hanjin / 1234
            </div>
          </div>
        </div>
      </div>
  ...
  const completed      = dispatches.filter(d=>["승인완료","수정확인요청","수정승인","수정거절"].includes(d.status));
  ...
  const needModConfirm = isCompany ? myDispatches.filter(d=>d.status==="수정확인요청").length : 0;
  const needAdminCheck = isAdmin && !isSuperAdmin ? dispatches.filter(d=>d.status==="수정승인").length : 0;
  ...
  // ── 공통 컴포넌트 ───────────────────────────────
  const Badge = ({st}:{st:string}) => <span style={s.badge(st)}>{st}</span>;
  ...
  const DispatchCard = ({d, adminView=false, companyView=false}: any) => {
    ...
    const bc = d.status==="승인완료"||d.status==="수정승인"?"#3B6D11":d.status==="업체거절"||d.status==="취소"||d.status==="수정거절"?"#ccc":d.status==="업체수락"?"#534AB7":d.status==="수정확인요청"?"#854F0B":"#378ADD";
    return (
      <div style={{...s.card,borderLeft:`4px solid ${bc}`}}>
        ...
        {companyView&&d.status==="요청중"&&<div>
          ...
          <div style={{display:"flex",gap:8}}>
            <button style={{...s.btn,...s.btnPu}} onClick={()=>{setStatus(d.id,"업체수락",{companyNote:note});addLog(currentUser.id,currentUser.name,"배차 수락",`${ship?.name} ${d.forklifts}대 수락`);}}>수락</button>
  ...
  const DispatchForm = () => (
    <div style={s.infoCard}>
      ...
      <button style={{...s.btn,...s.btnP}} onClick={()=>addDispatch(currentUser)}>요청 전송</button>
    </div>
  );
  ...
  return (
    <div style={s.wrap}>
      ...
      {/* 헤더 */}
      <div style={{background:headerBg,color:"#fff",padding:"14px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",borderRadius:"10px 10px 0 0"}}>
        <div>
          ...
          <div style={{fontSize:12,opacity:.8,marginTop:2,display:"flex",alignItems:"center",gap:6}}>
            ...
            <span style={{background:"rgba(255,255,255,0.2)",padding:"1px 8px",borderRadius:10,fontSize:11}}>{ROLE_STYLE[currentUser.role]?.label}</span>
          </div>
        </div>
      ...
      <div style={s.tabBar}>
        {tabs.map((t:string)=>{
          const badge = t==="최종승인"?needFinal : t==="배차 수락/거절"?needAction : t==="수정확인"?needModConfirm : t==="완료내역"?needAdminCheck : 0;
          return (
      ...
      <div style={s.body}>
        ...
        {isSuperAdmin&&activeTab==="대시보드"&&<>
          <div style={{...s.infoCard,borderColor:"#f5c6c6",background:"#fff9f9"}}><div style={{fontWeight:700,fontSize:15,color:"#7B1F1F",marginBottom:4}}>시스템 관리자 모드</div><div style={{fontSize:13,color:"#888"}}>전체 사용자 및 접속 로그를 관리합니다.</div></div>
          ...
          <div style={s.card}><table style={s.table}><thead><tr><th style={s.th}>시간</th><th style={s.th}>사용자</th><th style={s.th}>액션</th><th style={s.th}>상세</th></tr></thead><tbody>{logs.slice(0,10).map(l=><tr key={l.id}><td style={{...s.td,fontSize:12,color:"#888"}}>{l.time}</td><td style={{...s.td,fontWeight:600}}>{l.userName}</td><td style={s.td}><span style={{...(ACTION_STYLE[l.action]||{bg:"#eee",color:"#555"}),padding:"2px 8px",borderRadius:10,fontSize:12,fontWeight:600,display:"inline-block"}}>{l.action}</span></td><td style={{...s.td,fontSize:12,color:"#666"}}>{l.detail}</td></tr>)}</tbody></table></div>
        </>}

        {isSuperAdmin&&activeTab==="사용자관리"&&<>
          <div style={s.row}><div style={s.sec}>사용자 관리 ({users.length}명)</div><button style={{...s.btn,...s.btnP}} onClick={()=>setShowAddUser(!showAddUser)}>+ 사용자 추가</button></div>
          {showAddUser&&<div style={s.infoCard}>
            <div style={s.fgrid}>
              ...
              <div><label style={s.label}>권한</label><select style={s.input} value={newUser.role} onChange={e=>setNewUser({...newUser,role:e.target.value})}><option value="admin">관리자</option><option value="superadmin">시스템관리자</option></select></div>
            </div>
            {newUserErr&&<div style={{color:"#A32D2D",fontSize:13,marginBottom:10}}>{newUserErr}</div>}
            <div style={{display:"flex",gap:8}}><button style={{...s.btn,...s.btnP}} onClick={()=>addUserBySuperAdmin(currentUser)}>등록</button><button style={{...s.btn,...s.btnGray}} onClick={()=>{setShowAddUser(false);setNewUserErr("");}}>취소</button></div>
          </div>}
          <div style={s.card}><table style={s.table}><thead><tr><th style={s.th}>아이디</th><th style={s.th}>이름</th><th style={s.th}>권한</th><th style={s.th}>소속</th><th style={s.th}>관리</th></tr></thead><tbody>{users.map(u=>{ const comp=u.companyId?companies.find(c=>c.id===u.companyId):null; const rs=ROLE_STYLE[u.role]||{bg:"#eee",color:"#555",label:u.role}; return <tr key={u.id}><td style={{...s.td,fontWeight:600}}>{u.loginId}</td><td style={s.td}>{u.name}</td><td style={s.td}><span style={{...rs,padding:"2px 9px",borderRadius:12,fontSize:12,fontWeight:600,display:"inline-block"}}>{rs.label}</span></td><td style={{...s.td,fontSize:12,color:"#666"}}>{comp?.name||"—"}</td><td style={s.td}>{u.role!=="superadmin"&&<button style={{...s.btn,...s.btnD,padding:"3px 10px",fontSize:12}} onClick={()=>{ if(!window.confirm("삭제?"))return; const uu=users.find(x=>x.id===u.id); setUsers(p=>p.filter(x=>x.id!==u.id)); addLog(currentUser.id,currentUser.name,"업체 삭제",`${uu?.name} 삭제`); }}>삭제</button>}</td></tr>; })}</tbody></table></div>
        </>}
        ...
        {isSuperAdmin&&activeTab==="접속로그"&&<>
          <div style={{...s.row,flexWrap:"wrap" as const,gap:8}}>
            ...
            <div style={{display:"flex",gap:4,flexWrap:"wrap" as const}}>
              {["전체","로그인","로그아웃","배차 요청","배차 수락","최종 승인","배차 거절"].map(f=><button key={f} style={{...s.btn,padding:"5px 10px",fontSize:12,background:logFilter===f?"#1a3a5c":"#f0f0f0",color:logFilter===f?"#fff":"#555"}} onClick={()=>setLogFilter(f)}>{f}</button>)}
            </div>
          </div>
          <div style={s.card}><table style={s.table}><thead><tr><th style={s.th}>시간</th><th style={s.th}>사용자</th><th style={s.th}>액션</th><th style={s.th}>상세</th></tr></thead><tbody>{filteredLogs.map(l=><tr key={l.id}><td style={{...s.td,fontSize:12,color:"#888",whiteSpace:"nowrap" as const}}>{l.time}</td><td style={{...s.td,fontWeight:600}}>{l.userName}</td><td style={s.td}><span style={{...(ACTION_STYLE[l.action]||{bg:"#eee",color:"#555"}),padding:"2px 8px",borderRadius:10,fontSize:12,fontWeight:600,display:"inline-block"}}>{l.action}</span></td><td style={{...s.td,fontSize:12,color:"#555"}}>{l.detail}</td></tr>)}</tbody></table></div>
          <div style={{display:"flex",justifyContent:"flex-end"}}><button style={{...s.btn,...s.btnGreen}} onClick={()=>downloadCSV([["시간","사용자","액션","상세"],...filteredLogs.map(l=>[l.time,l.userName,l.action,l.detail])],"접속로그.csv")}>엑셀 다운로드</button></div>
        </>}
        ...
        {isSuperAdmin&&activeTab==="시스템현황"&&<>
          ...
          <div style={{...s.metric3}}>
            ...
            <div style={s.metric}><div style={{...s.metricVal,fontSize:15}}>{dispatches.reduce((s,d)=>s+calcAmount(d),0).toLocaleString()}원</div><div style={s.metricLbl}>전체 금액</div></div>
          </div>
        </>}
        ...
        {!isSuperAdmin&&isAdmin&&activeTab==="대시보드"&&<>
          ...
          <div style={s.card}><div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4}}>{getWeekDates("2026-04-03").map(date=>{ const dd=dispatches.filter(d=>d.date===date); return <div key={date} style={{background:dd.length>0?"#f0f4fa":"#fafafa",borderRadius:8,padding:"8px 6px",textAlign:"center" as const,border:"1px solid #e8e8e8"}}><div style={{fontSize:11,color:"#888",marginBottom:2}}>{getDayName(date)}</div><div style={{fontSize:12,fontWeight:600,color:"#1a3a5c",marginBottom:4}}>{date.slice(5)}</div><div style={{fontSize:18,fontWeight:700,color:dd.length>0?"#1a3a5c":"#ccc"}}>{dd.reduce((s,d)=>s+d.forklifts,0)}</div><div style={{fontSize:10,color:"#888"}}>대</div><div style={{fontSize:10,color:"#666",marginTop:2}}>{dd.length}건</div></div>; })}</div></div>
          <div style={s.sec}>업체별 배차 완료 현황</div>
          <div style={s.card}><table style={s.table}><thead><tr><th style={s.th}>업체명</th><th style={s.th}>완료건수</th><th style={s.th}>총 지게차</th><th style={s.th}>총 업무시간</th><th style={s.th}>완료금액</th></tr></thead><tbody>{companies.map(c=>{ const done=dispatches.filter(d=>d.companyId===c.id&&d.status==="승인완료"); return <tr key={c.id}><td style={{...s.td,fontWeight:600}}>{c.name}</td><td style={s.td}>{done.length}건</td><td style={s.td}>{done.reduce((s,d)=>s+d.forklifts,0)}대</td><td style={s.td}>{done.reduce((s,d)=>s+9*d.forklifts+d.extraHours*d.forklifts,0)}h</td><td style={{...s.td,fontWeight:600,color:"#1a3a5c"}}>{done.reduce((s,d)=>s+calcAmount(d),0).toLocaleString()}원</td></tr>; })}</tbody></table></div>
          ...
          <div style={{...s.card,background:"#f0f4fa",textAlign:"center" as const}}><div style={{fontSize:13,color:"#666",marginBottom:4}}>승인완료 금액 합계</div><div style={{fontSize:28,fontWeight:700,color:"#1a3a5c"}}>{dispatches.filter(d=>d.status==="승인완료").reduce((s,d)=>s+calcAmount(d),0).toLocaleString()}원</div></div>
        </>}

        {/* ===== 관리자: 주간계획표 ===== */}
        {!isSuperAdmin&&isAdmin&&activeTab==="주간계획표"&&<>
          <div style={s.row}><div style={s.sec}>주간 배차 계획표</div><input type="date" style={{...s.input,width:"auto"}} value={weekBase} onChange={e=>setWeekBase(e.target.value)}/></div>
          <div style={{display:"flex",flexDirection:"column" as const,gap:10,marginBottom:14}}>
            {weekDates.map(date=>{ const dd=dispatches.filter(d=>d.date===date); const totalF=dd.reduce((s,d)=>s+d.forklifts,0); const dayShips=ships.filter(sh=>sh.eta===date); return <div key={date} style={{...s.card,marginBottom:0}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:dd.length>0||dayShips.length>0?10:0}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{background:"#1a3a5c",color:"#fff",borderRadius:8,padding:"4px 10px",fontSize:13,fontWeight:700,minWidth:32,textAlign:"center" as const}}>{getDayName(date)}</div><div style={{fontSize:14,fontWeight:600,color:"#1a3a5c"}}>{date}</div></div><div style={{fontSize:13,fontWeight:700,color:totalF>0?"#1a3a5c":"#bbb"}}>{totalF>0?`총 ${totalF}대`:"배차 없음"}</div></div>{dayShips.map(sh=><div key={sh.id} style={{fontSize:12,color:"#185FA5",background:"#E6F1FB",borderRadius:6,padding:"5px 10px",marginBottom:6}}>🚢 {sh.name} · {sh.berth} · {sh.cargo}</div>)}{dd.length>0&&<div style={{display:"flex",flexDirection:"column" as const,gap:5}}>{companies.map(c=>{ const items=dd.filter(d=>d.companyId===c.id); if(items.length===0)return null; return <div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#f8f9fa",borderRadius:7,padding:"8px 12px"}}><span style={{fontSize:13,fontWeight:600,color:"#1a3a5c"}}>{c.name}</span><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:13,fontWeight:700,color:"#1a3a5c"}}>{items.reduce((s,d)=>s+d.forklifts,0)}대</span>{items.map(d=><span key={d.id} style={s.badge(d.status)}>{d.status}</span>)}</div></div>; })}</div>}</div>; })}
          </div>
          <div style={s.card}><div style={{fontWeight:600,fontSize:14,color:"#1a3a5c",marginBottom:10}}>업체별 주간 요약</div>{companies.map(c=>{ const items=dispatches.filter(d=>d.companyId===c.id&&weekDates.includes(d.date)); return <div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid #f0f0f0"}}><span style={{fontSize:13,fontWeight:600}}>{c.name}</span><div style={{display:"flex",gap:12,fontSize:13,color:"#555"}}><span>{items.length}건</span><span style={{fontWeight:700,color:"#1a3a5c"}}>{items.reduce((s,d)=>s+d.forklifts,0)}대</span></div></div>; })}<div style={{display:"flex",justifyContent:"space-between",marginTop:8,fontSize:13,fontWeight:700,color:"#1a3a5c",paddingTop:8,borderTop:"1px solid #e8e8e8"}}><span>주간 합계</span><span>{dispatches.filter(d=>weekDates.includes(d.date)).reduce((s,d)=>s+d.forklifts,0)}대</span></div></div>
        </>}
        ...
        {!isSuperAdmin&&isAdmin&&activeTab==="완료내역"&&<>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap" as const,gap:8}}>
            <div style={s.sec}>배차 완료 내역</div>
            <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap" as const}}>
              <select style={{...s.input,width:"auto"}} value={completedMonth} onChange={e=>setCompletedMonth(e.target.value)}>
                <option value="">전체 보기</option>
                {[...new Set(completed.map(d=>d.date.slice(0,7)))].sort().reverse().map(m=><option key={m} value={m}>{m}</option>)}
              </select>
              <button style={{...s.btn,...s.btnGreen}} onClick={()=>{
                const items = completedMonth ? completed.filter(d=>d.date.startsWith(completedMonth)) : completed;
                const header=["날짜","선박명","부두","업체명","지게차(대)","정규시간","추가시간(h)","금액(원)","상태"];
                const rows=items.map(d=>{ const ship=ships.find(s=>s.id===d.shipId); const comp=companies.find(c=>c.id===d.companyId); return [d.date,ship?.name||"",ship?.berth||"",comp?.name||"",d.forklifts,`${d.startTime}~${d.endTime}`,d.extraHours,calcAmount(d),d.status]; });
                rows.push(["","","","합계","","","",items.reduce((s,d)=>s+calcAmount(d),0),""]);
                downloadCSV([header,...rows],`완료내역_${completedMonth||"전체"}.csv`);
              }}>엑셀 다운로드</button>
            </div>
          </div>
          ...
          {(()=>{
            ...
            return <>
              <div style={s.metric3}>
                <div style={s.metric}><div style={s.metricVal}>{items.length}</div><div style={s.metricLbl}>완료 건수</div></div>
                <div style={s.metric}><div style={s.metricVal}>{items.filter(d=>d.status==="수정확인요청").length}</div><div style={s.metricLbl}>수정확인 대기</div></div>
                <div style={s.metric}><div style={{...s.metricVal,fontSize:15}}>{items.reduce((s,d)=>s+calcAmount(d),0).toLocaleString()}원</div><div style={s.metricLbl}>총 금액</div></div>
              </div>
              {items.length===0
                ?<div style={{...s.card,color:"#aaa",fontSize:13,textAlign:"center" as const,padding:30}}>해당 월 완료 내역이 없습니다.</div>
                :items.map(d=>{
                  const ship=ships.find(sh=>sh.id===d.shipId);
                  const comp=companies.find(c=>c.id===d.companyId);
                  const isEditingThis=editingDispatchId===d.id;
                  const borderColor=d.status==="수정확인요청"?"#854F0B":d.status==="수정승인"?"#3B6D11":d.status==="수정거절"?"#A32D2D":"#3B6D11";
                  return (
                    <div key={d.id} style={{...s.card,borderLeft:`4px solid ${borderColor}`}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                        <div><div style={{fontWeight:700,fontSize:14,color:"#1a3a5c"}}>{ship?.name}</div><div style={{fontSize:12,color:"#888",marginTop:2}}>{comp?.name} · {ship?.berth} · {d.date}</div></div>
                        <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap" as const}}>
                          <Badge st={d.status}/>
                          {!isEditingThis&&d.status!=="수정확인요청"&&<button style={{...s.btn,background:"#f0f4fa",color:"#1a3a5c",padding:"3px 10px",fontSize:12}} onClick={()=>{ setEditingDispatchId(d.id); setEditForm({forklifts:d.forklifts,startTime:d.startTime,endTime:d.endTime,extraHours:d.extraHours}); setModNote(""); }}>수정</button>}
                        </div>
                      </div>
                      {/* 원본 표시 */}
                      {(d.status==="수정확인요청"||d.status==="수정승인"||d.status==="수정거절")&&d.origForklifts!==undefined&&(
                        <div style={{background:"#fff9f0",border:"1px solid #f5c6a0",borderRadius:7,padding:"8px 12px",marginBottom:8,fontSize:12}}>
                          <div style={{fontWeight:600,color:"#854F0B",marginBottom:4}}>원본 내역</div>
                          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:4,color:"#666"}}>
                            <div>지게차: {d.origForklifts}대</div><div>{d.origStartTime}~{d.origEndTime}</div><div>추가: {d.origExtraHours}h</div>
                          </div>
                          {d.modNote&&<div style={{marginTop:4,color:"#854F0B"}}>수정 사유: {d.modNote}</div>}
                        </div>
                      )}
                      {isEditingThis?(
                        <div style={{background:"#f8fbff",border:"1px solid #b5d4f4",borderRadius:8,padding:"12px",marginBottom:8}}>
                          <div style={{fontSize:12,fontWeight:600,color:"#185FA5",marginBottom:8}}>업무 내역 수정</div>
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                            <div><label style={s.label}>지게차(대)</label><input style={s.input} type="number" min="1" value={editForm.forklifts} onChange={e=>setEditForm({...editForm,forklifts:e.target.value})}/></div>
                            <div><label style={s.label}>추가근무(h)</label><input style={s.input} type="number" min="0" value={editForm.extraHours} onChange={e=>setEditForm({...editForm,extraHours:e.target.value})}/></div>
                            <div><label style={s.label}>시작 시간</label><input style={s.input} type="time" value={editForm.startTime} onChange={e=>setEditForm({...editForm,startTime:e.target.value})}/></div>
                            <div><label style={s.label}>종료 시간</label><input style={s.input} type="time" value={editForm.endTime} onChange={e=>setEditForm({...editForm,endTime:e.target.value})}/></div>
                          </div>
                          <div style={{marginBottom:8}}><label style={s.label}>수정 사유 *</label><input style={s.input} value={modNote} onChange={e=>setModNote(e.target.value)} placeholder="수정 사유 입력 (업체에 전달됨)"/></div>
                          <div style={{fontSize:13,color:"#1a3a5c",fontWeight:600,marginBottom:8,background:"#f0f4fa",padding:"8px 10px",borderRadius:6}}>
                            수정 후 금액: {(9*Number(editForm.forklifts)*d.unitPrice+Number(editForm.extraHours)*Number(editForm.forklifts)*Math.round(d.unitPrice*1.5)).toLocaleString()}원
                            <span style={{fontSize:12,color:"#888",marginLeft:8}}>(기존: {calcAmount(d).toLocaleString()}원)</span>
                          </div>
                          <div style={{display:"flex",gap:6}}>
                            <button style={{...s.btn,...s.btnP}} onClick={()=>submitDispatchEdit(d,currentUser)} disabled={!modNote}>수정요청 전송</button>
                            <button style={{...s.btn,...s.btnGray}} onClick={()=>setEditingDispatchId(null)}>취소</button>
                          </div>
                        </div>
                      ):(
                        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6,fontSize:13,color:"#555",background:"#f8f9fa",padding:"10px 12px",borderRadius:8,marginBottom:8}}>
                          <div>지게차: <b>{d.forklifts}대</b></div><div>업무: <b>{d.startTime}~{d.endTime}</b></div>
                          <div>추가근무: <b>{d.extraHours}h</b></div><div>총 시간: <b>{9*d.forklifts+d.extraHours*d.forklifts}h</b></div>
                        </div>
                      )}
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <div style={{fontSize:12,color:"#888"}}>{d.status==="수정확인요청"?"업체 확인 대기 중":d.status==="수정승인"?"업체 수정 승인완료":d.status==="수정거절"?"업체가 수정 거절함":""}</div>
                        <div style={{fontSize:14,fontWeight:700,color:"#1a3a5c"}}>{calcAmount(d).toLocaleString()}원</div>
                      </div>
                    </div>
                  );
                })
              ...
              <div style={{...s.card,background:"#f0f4fa",textAlign:"center" as const}}>
                <div style={{fontSize:13,color:"#666",marginBottom:4}}>{completedMonth||"전체"} 합계</div>
                <div style={{fontSize:26,fontWeight:700,color:"#1a3a5c"}}>{items.reduce((s,d)=>s+calcAmount(d),0).toLocaleString()}원</div>
              </div>
            </>;
          })()}
        </>}
        ...
        {!isSuperAdmin&&isAdmin&&activeTab==="입항계획"&&<>
          ...
          {showAddShip&&<div style={s.infoCard}><div style={s.fgrid}><div><label style={s.label}>선박명</label><input style={s.input} value={newShip.name} onChange={e=>setNewShip({...newShip,name:e.target.value})}/></div><div><label style={s.label}>접안 부두</label><input style={s.input} value={newShip.berth} onChange={e=>setNewShip({...newShip,berth:e.target.value})}/></div><div><label style={s.label}>입항 예정일</label><input style={s.input} type="date" value={newShip.eta} onChange={e=>setNewShip({...newShip,eta:e.target.value})}/></div><div><label style={s.label}>화물 정보</label><input style={s.input} value={newShip.cargo} onChange={e=>setNewShip({...newShip,cargo:e.target.value})}/></div></div><button style={{...s.btn,...s.btnP}} onClick={addShip}>등록</button></div>}
        ...
        {!isSuperAdmin&&isAdmin&&activeTab==="업체관리"&&<>
          ...
          {showAddCompany&&<div style={s.infoCard}>
            ...
            <div style={{display:"flex",gap:8}}><button style={{...s.btn,...s.btnP}} onClick={()=>addCompanyByAdmin(currentUser)}>등록</button><button style={{...s.btn,...s.btnGray}} onClick={()=>{setShowAddCompany(false);setNewCompanyErr("");}}>취소</button></div>
          </div>}
          {companies.map(c=>{ const compUser=users.find(u=>u.companyId===c.id); return <div key={c.id} style={s.card}>
            {editingCompany?.id===c.id?(
              <div>
                ...
                <div style={{marginBottom:10}}>
                  ...
                  {editingCompany.vehicles.map((v:any,i:number)=><div key={i} style={{display:"flex",gap:8,marginBottom:6,alignItems:"center"}}>
              ...
              <div>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:10,alignItems:"flex-start"}}>
                  ...
                  <div style={{display:"flex",gap:6}}>
                    ...
                    <button style={{...s.btn,...s.btnD,padding:"4px 12px",fontSize:12}} onClick={()=>deleteCompany(c.id,currentUser)}>삭제</button>
                  </div>
                </div>
                ...
                {c.vehicles?.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap" as const,marginBottom:8}}>{c.vehicles.map((v:any,i:number)=><span key={i} style={{fontSize:12,background:"#f0f4fa",color:"#1a3a5c",padding:"3px 10px",borderRadius:12,fontWeight:600}}>{v.type} {v.count}대</span>)}</div>}
        ...
        {/* ===== 협력업체: 수정확인 ===== */}
        {isCompany&&activeTab==="수정확인"&&<>
          <div style={s.sec}>수정 확인 요청</div>
          {(()=>{
            const modItems=myDispatches.filter(d=>["수정확인요청","수정승인","수정거절"].includes(d.status));
            if(modItems.length===0) return <div style={{...s.card,color:"#aaa",fontSize:13,textAlign:"center" as const,padding:30}}>수정 요청이 없습니다.</div>;
            return modItems.map(d=>{
              const ship=ships.find(sh=>sh.id===d.shipId);
              const borderColor=d.status==="수정확인요청"?"#854F0B":d.status==="수정승인"?"#3B6D11":"#A32D2D";
              const origAmt=9*(d.origForklifts||d.forklifts)*d.unitPrice+(d.origExtraHours||0)*(d.origForklifts||d.forklifts)*Math.round(d.unitPrice*1.5);
              const newAmt=calcAmount(d); const diff=newAmt-origAmt;
              return (
                <div key={d.id} style={{...s.card,borderLeft:`4px solid ${borderColor}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                    <div><div style={{fontWeight:700,fontSize:14,color:"#1a3a5c"}}>{ship?.name}</div><div style={{fontSize:12,color:"#888",marginTop:2}}>{ship?.berth} · {d.date}</div></div>
                    <Badge st={d.status}/>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
                    <div style={{background:"#fff9f0",border:"1px solid #f5c6a0",borderRadius:8,padding:"10px 12px"}}>
                      <div style={{fontSize:11,fontWeight:600,color:"#854F0B",marginBottom:6}}>변경 전</div>
                      <div style={{fontSize:13,color:"#555"}}>
                        <div>지게차: <b>{d.origForklifts||d.forklifts}대</b></div>
                        <div>업무: <b>{d.origStartTime||d.startTime}~{d.origEndTime||d.endTime}</b></div>
                        <div>추가: <b>{d.origExtraHours||0}h</b></div>
                        <div style={{marginTop:4,fontWeight:600,color:"#555"}}>{origAmt.toLocaleString()}원</div>
                      </div>
                    </div>
                    <div style={{background:"#f0f7ff",border:"1px solid #b5d4f4",borderRadius:8,padding:"10px 12px"}}>
                      <div style={{fontSize:11,fontWeight:600,color:"#185FA5",marginBottom:6}}>변경 후</div>
                      <div style={{fontSize:13,color:"#555"}}>
                        <div>지게차: <b>{d.forklifts}대</b></div>
                        <div>업무: <b>{d.startTime}~{d.endTime}</b></div>
                        <div>추가: <b>{d.extraHours}h</b></div>
                        <div style={{marginTop:4,fontWeight:600,color:"#1a3a5c"}}>{newAmt.toLocaleString()}원</div>
                      </div>
                    </div>
                  </div>
                  <div style={{background:diff>=0?"#EAF3DE":"#FCEBEB",borderRadius:7,padding:"8px 12px",marginBottom:10,fontSize:13,fontWeight:600,color:diff>=0?"#3B6D11":"#A32D2D",textAlign:"center" as const}}>
                    금액 변동: {diff>=0?"+":""}{diff.toLocaleString()}원
                  </div>
                  {d.modNote&&<div style={{fontSize:12,color:"#854F0B",background:"#fff9f0",padding:"6px 10px",borderRadius:6,marginBottom:10}}>수정 사유: {d.modNote}</div>}
                  {d.status==="수정확인요청"&&<div style={{display:"flex",gap:8}}>
                    <button style={{...s.btn,...s.btnS,flex:1}} onClick={()=>{ setStatus(d.id,"수정승인"); addLog(currentUser.id,currentUser.name,"배차 수락",`수정내역 승인: ${ship?.name}`); }}>수정 승인</button>
                    <button style={{...s.btn,...s.btnD,flex:1}} onClick={()=>{ setStatus(d.id,"수정거절"); addLog(currentUser.id,currentUser.name,"배차 거절",`수정내역 거절: ${ship?.name}`); }}>수정 거절</button>
                  </div>}
                  {d.status==="수정승인"&&<div style={{fontSize:13,color:"#3B6D11",background:"#EAF3DE",padding:"8px 12px",borderRadius:7}}>수정 내용을 승인하셨습니다.</div>}
                  {d.status==="수정거절"&&<div style={{fontSize:13,color:"#A32D2D",background:"#FCEBEB",padding:"8px 12px",borderRadius:7}}>수정 내용을 거절하셨습니다.</div>}
                </div>
              );
            });
          })()}
        ...
        {activeTab==="월별정산"&&<>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap" as const,gap:8}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              {isAdmin&&<button style={{...s.btn,...s.btnBlue}} onClick={()=>setActiveTab("완료내역")}>완료내역 →</button>}
            </div>
          </div>
          {isAdmin&&<div style={s.metric3}>
            <div style={s.metric}><div style={s.metricVal}>{dispatches.filter(d=>d.date.startsWith(selectedMonth)&&d.status==="승인완료").length}</div><div style={s.metricLbl}>완료 건수</div></div>
            <div style={s.metric}><div style={s.metricVal}>{dispatches.filter(d=>d.date.startsWith(selectedMonth)&&d.status==="요청중").length}</div><div style={s.metricLbl}>진행 중</div></div>
            <div style={s.metric}><div style={{...s.metricVal,fontSize:15}}>{dispatches.filter(d=>d.date.startsWith(selectedMonth)&&d.status==="승인완료").reduce((s,d)=>s+calcAmount(d),0).toLocaleString()}원</div><div style={s.metricLbl}>완료 금액</div></div>
          </div>}
          {monthlyData.map((c:any)=>(
            <div key={c.id} style={s.card}>
              ...
              {c.items.length===0?<div style={{color:"#aaa",fontSize:13}}>해당 월 배차 내역 없음</div>:
                c.items.map((d:any)=>{ const ship=ships.find((sh:any)=>sh.id===d.shipId); return (
                  <div key={d.id} style={{background:"#f8f9fa",borderRadius:8,padding:"10px 12px",marginBottom:8}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:4,fontSize:12,color:"#555"}}>
                      <div>{d.date}</div><div>{d.forklifts}대 · {d.extraHours}h추가</div>
                      <div style={{fontWeight:600,color:"#1a3a5c",textAlign:"right" as const}}>{calcAmount(d).toLocaleString()}원</div>
                    </div>
                  </div>
                ); })
              }
              <div style={{marginTop:8,display:"flex",justifyContent:"space-between",fontSize:13,color:"#666",borderTop:"1px solid #eee",paddingTop:8}}>
                <span>총 업무시간: <b>{c.hrs}h</b></span><span>건수: <b>{c.items.length}건</b></span>
              </div>
            </div>
          ))}
          <div style={{...s.card,background:"#f0f4fa",textAlign:"center" as const}}>
